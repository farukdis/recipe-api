...
exports.updateComment = async (req, res) => {
  try {
    ...
    if (comment.user_id !== userId) {
      return res.status(403).json({ message: "Bu yorumu güncelleme izniniz yok." });
    }
...
exports.deleteComment = async (req, res) => {
  try {
    ...
    // Yorumun var olup olmadığını ve kullanıcının yorumun sahibi olup olmadığını kontrol et
    const comment = await knex('comments').where({ id }).first();
    ...
    if (comment.user_id !== userId) {
      return res.status(403).json({ message: "Bu yorumu silme izniniz yok." });
    }
